...
The `home-routes` handler in the `green-eye.routes.home` namespace
...
The `home-page` function will in turn call the `green-eye.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `green-eye.handler` namespace:
...
Request middleware functions are located under the `green-eye.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `green-eye.dev-middleware` namespace found in
